...
/*
    ...
    Purpose: Shuffles the current deck using the Fisher-Yates algorithm.
    Arguments:
...
void shuffleDeck(Card* deck, int size) {
	...
	/* Algorithm starts from the last element */
    for (i = size - 1; i > 0; i--) {
        j = rand() % (i + 1); /* 0 <= j <= i */
        Card tmp = deck[i];
        deck[i] = deck[j];
...
HandValue rankHand(Card hand[2], Card community[5]) {
	...
	/* Puts all the cards in one array */
    for (i = 0; i < 2; i++) {
        all[i] = hand[i];
    }
	...
	// Checking for suits to be able to count for flush, full house, etc
    for (i = 0; i < 7; i++) {
    ...
    /* Determine hand type */
	/* Royal flush */
	if (straightHigh && straightHigh == 10) {
		hv.rank = ROYAL_FLUSH;
		hv.tiebreak[0] = ROYAL_FLUSH;
		hv.tiebreak[1] = ROYAL_FLUSH;
	/* Straight flush */
	} else if (straightHigh && flushSuit) {
		hv.rank = STRAIGHT_FLUSH;
		hv.tiebreak[0] = STRAIGHT_FLUSH;
		hv.tiebreak[1] = STRAIGHT_FLUSH;
	/* Four of a kind */
    } else if (fours) {
...
/*
    Function: showHand
    Purpose: Shows the players; hand, prints it
    Arguments:
...
const char* getSuitSymbol(char *suit) {
    if (equalStrings(suit, "Hearts") == 0) {
        return "♥";
    }

    if (equalStrings(suit, "Diamonds") == 0) {
        return "♦";
    }

    if (equalStrings(suit, "Clubs") == 0) {
        return "♣";
    }

    if (equalStrings(suit, "Spades") == 0) {
        return "♠";
    }
